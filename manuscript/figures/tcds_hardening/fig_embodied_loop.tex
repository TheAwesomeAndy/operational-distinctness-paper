...
\node[env, text width=58mm]  (env)  at (0,3.4)    {Recorded ERP observation bank /\\simulated affective-state process\\[2pt]{\scriptsize held-out subject-condition observation}\\[-1pt]{\scriptsize simulated affective-state transition}};
\node[perc, text width=40mm] (perc) at (7.4,3.4)  {ARSPI-Net evidence estimator\\[1pt]{\scriptsize substrate $\rightarrow$ belief $b(s)$}\\[-1pt]{\scriptsize Dirichlet update\ $\boldsymbol{\alpha}\!\leftarrow\!\boldsymbol{\alpha}\!+\!\mathbf{p}_{\theta}(x_t)$}};
...
\draw[flow] (efe) to[bend left=14] node[left,font=\scriptsize,align=center,pos=0.5]{policy action /\\ target-state query\\{\scriptsize until $b(\text{target})\!\ge\!0.8$}} (env);

% ---- neural evidence streams feeding the decoder ----
...
\node[font=\scriptsize,align=center,text=black!60] at (10.9,4.9){neural evidence streams};
...
\node[font=\scriptsize,align=center,text=black!60] at (3.7,-2.35)
  {Epistemic (ambiguity) term adds limited marginal utility under the simulated affective-state transition:\\[-1pt]EFE controller matches pragmatic-only control. Closed loop on the belief side, open loop on the\\[-1pt]process side; evaluated as a simulation over recorded observations.};
